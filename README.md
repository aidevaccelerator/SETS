...
```bash
python tools/fetch_data.py --hours 2400                    # BTCUSDT → dist/data/candles.BTCUSDT.js
python tools/fetch_data.py --symbol ETHUSDT --hours 2400   # ETHUSDT → dist/data/candles.ETHUSDT.js
python tools/fetch_data.py --symbol SOLUSDT --hours 2400   # SOLUSDT → dist/data/candles.SOLUSDT.js
```

### Markets: BTC · ETH · SOL

Use the **BTC / ETH / SOL** toggle in the control bar (or `?sym=ETH` / `?sym=SOL` in the URL). Each market has its own bundled tape, its own evolution, and its own frozen forward cohort — switching swaps all three atomically. Same seed on a different tape is a different experiment.

### Forward test (live paper)

Evolution can overfit the bundled tape. The **forward test** freezes the top-K gate-passing genomes at T0 (the last bundled candle) and replays them on **live hourly candles fetched in the browser from Binance's public API** — no re-evolution, only bars the cohort has never seen. Buy & hold on the same bars is the baseline.
...
tools/fetch_data.py   Writes dist/data/candles.<SYMBOL>.js from Binance's public API
tools/make_snapshot.mjs  Freezes the top-K genomes into dist/data/snapshot.<SYMBOL>.js
tests/                Node test runner: data, indicators, bot mechanics, GA, forward test
docs/                 README images, GIFs and the promo video
```

Built with plain HTML, CSS and JavaScript modules on `<canvas>`. No frameworks; the only runtime network request is the forward test's public Binance klines fetch (no key).
